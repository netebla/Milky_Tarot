...
Тестовый сценарий "Мои рыбки" (для админов)
-------------------------------------------

Для демонстрации платёжного сценария добавлен тестовый поток внутренней валюты — "рыбки":

- В главном меню у админов появляется кнопка `Мои рыбки`.
- По нажатию бот показывает текущий баланс пользователя в поле `fish_balance` таблицы `users` и предлагает:
  - вернуться в главное меню;
  - пополнить баланс.
- Пополнение:
  - бот показывает тарифы (50₽ → 350 🐟, 150₽ → 1050 🐟, 300₽ → 2100 🐟, 650₽ → 4550 🐟) и инлайн-кнопки с выбором тарифа;
  - далее предлагаются способы оплаты (СБП, карта, звёзды Telegram);
  - после выбора способа оплаты бот имитирует успешную оплату и начисляет рыбки (включая бонусные) в `fish_balance`, сообщая новый баланс.
- На каждом шаге есть возможность:
...
Для уже развёрнутой PostgreSQL-базы нужно выполнить миграцию вручную:

```sql
ALTER TABLE users ADD COLUMN IF NOT EXISTS fish_balance integer DEFAULT 0;
```

Если база запущена через `docker-compose.yml` из этого репозитория, пример команд на ВМ:
...
После этого можно перезапустить бота:
